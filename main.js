...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Raven ❤️ Alveus" }, status: "idle" });
  let botVoiceChannel = client.channels.cache.get(ayarlar.botVoiceChannelID);
  if (botVoiceChannel) botVoiceChannel.join().catch(err => console.error("Bot ses kanalına bağlanamadı!"));
});
// Raven ❤️ Alveus tarafından kodlanmıştır.
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#00ffdd").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Raven ❤️ Alveus"} was here!`).setTimestamp();
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Raven ❤️ Alveus" }).catch();
};
...
client.on("guildMemberRemove", async member => {
  ...
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Kick Atıldı!').setDescription(`${member} **(${member.id})** üyesi, ${entry.executor} **(${entry.executor.id})** tarafından sunucudan sağ tık ile kicklendi! Kickleyen kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Raven ❤️ Alveus"} was here!`).setTimestamp()).catch(); } else { member.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Kick Atıldı!').setDescription(`${member} **(${member.id})** üyesi, ${entry.executor} (${entry.executor.id}) tarafından sunucudan sağ tık ile kicklendi! Kickleyen kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Raven ❤️ Alveus"} was here!`).setTimestamp()).catch(err => {}); };
});
// Ban koruması
...
client.on("channelDelete", async channel => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
  cezalandir(entry.executor.id, "ban");
  await channel.clone({ reason: "Raven ❤️ Alveus" }).then(async kanal => {
